...
- `src/pages/` — ファイルベースルーティング。`src/pages/`直下にフラットに置かれた`<name>.md`がそのままそのルートに対応する(例: `src/pages/profile.md` → `/profile`)。`<name>/index.md`という同名フォルダ+`index.md`の形は使わない。全ページがMarkdownで書かれており、frontmatterの`layout`で`src/layouts/Layout.astro`への相対パスを指定し、`title`/`description`もfrontmatterで渡す。本文はCommonMarkで記述し、素のMarkdownで表現できない部分(画像タグなど)は生HTMLをそのまま埋め込める。Astro標準のShikiによりフェンスコードブロックが自動でハイライトされる(`astro.config.mjs`の`markdown.shikiConfig`参照)。ページ内に`## 目次`という見出しを書くと、remark-toc(`astro.config.mjs`の`markdown.processor`)がその直下に見出し一覧へのリンクを自動生成する(例: `src/pages/portfolio.md`)。見出しだけ書いてリスト本体が空に見えるのは、この自動生成を待っている状態であり不具合ではない。
