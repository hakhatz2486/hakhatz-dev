...
出典: [morhetz/gruvbox-contrib - color.table](https://github.com/morhetz/gruvbox-contrib/blob/master/color.table)

`public/css/style.css`のCSSカスタムプロパティ(ライト/ダークテーマ)は、このパレットの値を元にしている。「Dark Alias」「Light Alias」は各テーマでその色が担う役割(`bg0`/`fg0`など)、「Dark 256」「Light 256」は各テーマでのターミナル256色パレット上のインデックスを表す。

| Gruvbox Color | Hex | Dark Alias | Light Alias | Term Color | Dark 256 | Light 256 | RGB | iTerm RGB | macOS Hex |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
